...
if [[ "$1" == "bioinformatics" ]]; then
  Q="Bioinformatics engineering — sequence alignment, variant calling pipelines, genomic data formats, and cloud-scale genomics?"; AGENTS=(Octavia Lucidia Alice Aria Shellfish); OUT="$SAVE_DIR/bioinformatics-$(date +%s).txt"
  echo "🧬 Bioinformatics Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Bioinformatics roundtable complete."; exit 0; fi

if [[ "$1" == "health-data-eng" ]]; then
  Q="Healthcare data engineering — FHIR, HL7, HIPAA compliance, EHR integration, and interoperability challenges?"; AGENTS=(Lucidia Alice Octavia Shellfish Aria); OUT="$SAVE_DIR/health-data-eng-$(date +%s).txt"
  echo "🏥 Health Data Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Health data engineering roundtable complete."; exit 0; fi

if [[ "$1" == "clinical-ai" ]]; then
  Q="AI in clinical settings — model validation, FDA clearance, bias in medical AI, and deployment in care workflows?"; AGENTS=(Aria Octavia Shellfish Lucidia Alice); OUT="$SAVE_DIR/clinical-ai-$(date +%s).txt"
  echo "💊 Clinical AI Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Clinical AI roundtable complete."; exit 0; fi

if [[ "$1" == "wearables-eng" ]]; then
  Q="Engineering wearable health devices — BLE low-power, biosensor signal processing, continuous monitoring, and data sync?"; AGENTS=(Shellfish Lucidia Aria Octavia Alice); OUT="$SAVE_DIR/wearables-eng-$(date +%s).txt"
  echo "⌚ Wearables Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Wearables engineering roundtable complete."; exit 0; fi

if [[ "$1" == "drug-discovery-ai" ]]; then
  Q="AI for drug discovery — protein folding, molecular generation, ADMET prediction, and accelerating clinical trials?"; AGENTS=(Alice Aria Lucidia Shellfish Octavia); OUT="$SAVE_DIR/drug-discovery-ai-$(date +%s).txt"
  echo "🔬 Drug Discovery AI Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Drug discovery AI roundtable complete."; exit 0; fi

if [[ "$1" == "space-software" ]]; then
  Q="Software for space systems — radiation-hardened code, fault tolerance, on-board vs ground systems, and SpaceX/NASA patterns?"; AGENTS=(Octavia Shellfish Lucidia Alice Aria); OUT="$SAVE_DIR/space-software-$(date +%s).txt"
  echo "🚀 Space Software Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Space software roundtable complete."; exit 0; fi

if [[ "$1" == "satellite-systems" ]]; then
  Q="Satellite ground systems engineering — link budgets, TT&C, on-orbit software updates, and NewSpace vs legacy?"; AGENTS=(Lucidia Alice Octavia Aria Shellfish); OUT="$SAVE_DIR/satellite-systems-$(date +%s).txt"
  echo "🛰️ Satellite Systems Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Satellite systems roundtable complete."; exit 0; fi

if [[ "$1" == "aerospace-safety" ]]; then
  Q="DO-178C, DO-254, and aerospace safety standards — how to build software that can fly and survive certification?"; AGENTS=(Aria Octavia Alice Shellfish Lucidia); OUT="$SAVE_DIR/aerospace-safety-$(date +%s).txt"
  echo "✈️ Aerospace Safety Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Aerospace safety roundtable complete."; exit 0; fi

if [[ "$1" == "autonomous-vehicles" ]]; then
  Q="Autonomous vehicle stacks — perception, localization, planning, prediction, and why L4 is still so hard?"; AGENTS=(Shellfish Lucidia Octavia Aria Alice); OUT="$SAVE_DIR/autonomous-vehicles-$(date +%s).txt"
  echo "🚗 Autonomous Vehicles Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Autonomous vehicles roundtable complete."; exit 0; fi

if [[ "$1" == "robotics-ros" ]]; then
  Q="ROS2 and modern robotics software — real-time control, SLAM, motion planning, simulation with Gazebo, and hardware interfaces?"; AGENTS=(Alice Aria Shellfish Octavia Lucidia); OUT="$SAVE_DIR/robotics-ros-$(date +%s).txt"
  echo "🤖 Robotics ROS Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Robotics ROS roundtable complete."; exit 0; fi

if [[ "$1" == "defi-engineering" ]]; then
  Q="DeFi protocol engineering — smart contract security, MEV, liquidity mechanics, and on-chain governance systems?"; AGENTS=(Octavia Alice Lucidia Shellfish Aria); OUT="$SAVE_DIR/defi-engineering-$(date +%s).txt"
  echo "💸 DeFi Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ DeFi engineering roundtable complete."; exit 0; fi

if [[ "$1" == "smart-contracts" ]]; then
  Q="Smart contract development — Solidity patterns, reentrancy, gas optimization, formal verification, and audit checklists?"; AGENTS=(Lucidia Shellfish Alice Octavia Aria); OUT="$SAVE_DIR/smart-contracts-$(date +%s).txt"
  echo "📜 Smart Contracts Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Smart contracts roundtable complete."; exit 0; fi

if [[ "$1" == "blockchain-scaling" ]]; then
  Q="Blockchain scaling — rollups vs sidechains, data availability, ZK vs optimistic rollups, and modular blockchains?"; AGENTS=(Aria Alice Octavia Lucidia Shellfish); OUT="$SAVE_DIR/blockchain-scaling-$(date +%s).txt"
  echo "⛓️ Blockchain Scaling Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Blockchain scaling roundtable complete."; exit 0; fi

if [[ "$1" == "web3-ux" ]]; then
  Q="Web3 UX engineering — wallet abstraction, gasless transactions, progressive decentralization, and onboarding non-crypto users?"; AGENTS=(Shellfish Octavia Aria Alice Lucidia); OUT="$SAVE_DIR/web3-ux-$(date +%s).txt"
  echo "🌐 Web3 UX Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Web3 UX roundtable complete."; exit 0; fi

if [[ "$1" == "nft-infrastructure" ]]; then
  Q="NFT infrastructure — IPFS vs Arweave, metadata standards, royalty enforcement, and building resilient token ecosystems?"; AGENTS=(Alice Lucidia Shellfish Aria Octavia); OUT="$SAVE_DIR/nft-infrastructure-$(date +%s).txt"
  echo "🖼️ NFT Infrastructure Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ NFT infrastructure roundtable complete."; exit 0; fi

if [[ "$1" == "fintech-core" ]]; then
  Q="Core banking modernization — ledger architecture, double-entry systems, idempotent payments, and reconciliation at scale?"; AGENTS=(Octavia Aria Alice Shellfish Lucidia); OUT="$SAVE_DIR/fintech-core-$(date +%s).txt"
  echo "🏦 Fintech Core Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Fintech core roundtable complete."; exit 0; fi

if [[ "$1" == "fraud-detection" ]]; then
  Q="Real-time fraud detection — feature engineering, graph-based detection, model serving latency, and false positive management?"; AGENTS=(Lucidia Octavia Shellfish Alice Aria); OUT="$SAVE_DIR/fraud-detection-$(date +%s).txt"
  echo "🚨 Fraud Detection Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Fraud detection roundtable complete."; exit 0; fi

if [[ "$1" == "high-frequency-trading" ]]; then
  Q="HFT systems engineering — kernel bypass networking, FPGA co-location, nanosecond latency, and market data feeds?"; AGENTS=(Aria Shellfish Alice Octavia Lucidia); OUT="$SAVE_DIR/high-frequency-trading-$(date +%s).txt"
  echo "📊 High-Frequency Trading Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ High-frequency trading roundtable complete."; exit 0; fi

if [[ "$1" == "risk-systems" ]]; then
  Q="Financial risk systems — VaR, stress testing, margin calculation, real-time position limits, and regulatory capital?"; AGENTS=(Shellfish Alice Lucidia Aria Octavia); OUT="$SAVE_DIR/risk-systems-$(date +%s).txt"
  echo "⚠️ Risk Systems Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Risk systems roundtable complete."; exit 0; fi

if [[ "$1" == "open-banking" ]]; then
  Q="Open banking APIs — PSD2, OAuth consent flows, screen scraping vs direct API, and financial data aggregation patterns?"; AGENTS=(Alice Octavia Aria Lucidia Shellfish); OUT="$SAVE_DIR/open-banking-$(date +%s).txt"
  echo "🏧 Open Banking Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Open banking roundtable complete."; exit 0; fi

if [[ "$1" == "climate-tech-eng" ]]; then
  Q="Engineering for climate tech — carbon accounting systems, grid software, energy optimization, and measurement/reporting infra?"; AGENTS=(Octavia Lucidia Aria Alice Shellfish); OUT="$SAVE_DIR/climate-tech-eng-$(date +%s).txt"
  echo "🌱 Climate Tech Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Climate tech engineering roundtable complete."; exit 0; fi

if [[ "$1" == "energy-grid-software" ]]; then
  Q="Smart grid software — SCADA modernization, demand response, battery storage management, and renewable integration?"; AGENTS=(Lucidia Shellfish Octavia Aria Alice); OUT="$SAVE_DIR/energy-grid-software-$(date +%s).txt"
  echo "⚡ Energy Grid Software Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Energy grid software roundtable complete."; exit 0; fi

if [[ "$1" == "supply-chain-eng" ]]; then
  Q="Supply chain engineering — inventory optimization, demand forecasting, supplier APIs, and building resilient logistics systems?"; AGENTS=(Aria Alice Shellfish Lucidia Octavia); OUT="$SAVE_DIR/supply-chain-eng-$(date +%s).txt"
  echo "📦 Supply Chain Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Supply chain engineering roundtable complete."; exit 0; fi

if [[ "$1" == "edtech-eng" ]]; then
  Q="EdTech platform engineering — adaptive learning systems, spaced repetition, video infra, and engagement mechanics?"; AGENTS=(Shellfish Octavia Lucidia Alice Aria); OUT="$SAVE_DIR/edtech-eng-$(date +%s).txt"
  echo "📚 EdTech Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ EdTech engineering roundtable complete."; exit 0; fi

if [[ "$1" == "legaltech-eng" ]]; then
  Q="LegalTech engineering — contract analysis AI, e-discovery pipelines, court filing systems, and legal data privacy?"; AGENTS=(Alice Aria Octavia Shellfish Lucidia); OUT="$SAVE_DIR/legaltech-eng-$(date +%s).txt"
  echo "⚖️ LegalTech Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ LegalTech engineering roundtable complete."; exit 0; fi

if [[ "$1" == "proptech-eng" ]]; then
  Q="PropTech engineering — MLS integrations, property valuation models, IoT smart buildings, and real estate data pipelines?"; AGENTS=(Octavia Lucidia Shellfish Aria Alice); OUT="$SAVE_DIR/proptech-eng-$(date +%s).txt"
  echo "🏠 PropTech Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ PropTech engineering roundtable complete."; exit 0; fi

if [[ "$1" == "adtech-eng" ]]; then
  Q="Ad tech engineering — bidding systems, RTB latency, cookie deprecation, privacy-preserving targeting, and attribution?"; AGENTS=(Lucidia Alice Aria Octavia Shellfish); OUT="$SAVE_DIR/adtech-eng-$(date +%s).txt"
  echo "📣 AdTech Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ AdTech engineering roundtable complete."; exit 0; fi

if [[ "$1" == "media-streaming-eng" ]]; then
  Q="Video streaming engineering — ABR algorithms, CDN strategy, live vs VOD pipelines, codec selection, and low-latency HLS?"; AGENTS=(Aria Shellfish Alice Lucidia Octavia); OUT="$SAVE_DIR/media-streaming-eng-$(date +%s).txt"
  echo "📺 Media Streaming Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Media streaming engineering roundtable complete."; exit 0; fi

if [[ "$1" == "lsp-servers" ]]; then
  Q="Language Server Protocol engineering — how to build an LSP, incremental parsing, semantic tokens, and IDE integration patterns?"; AGENTS=(Octavia Lucidia Alice Aria Shellfish); OUT="$SAVE_DIR/lsp-servers-$(date +%s).txt"
  echo "🔧 LSP Servers Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ LSP servers roundtable complete."; exit 0; fi

if [[ "$1" == "debugger-eng" ]]; then
  Q="Debugger internals — DWARF debug info, ptrace, DAP protocol, time-travel debugging, and building custom debugger tooling?"; AGENTS=(Lucidia Shellfish Octavia Alice Aria); OUT="$SAVE_DIR/debugger-eng-$(date +%s).txt"
  echo "🐛 Debugger Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Debugger engineering roundtable complete."; exit 0; fi

if [[ "$1" == "profiler-eng" ]]; then
  Q="Profiler engineering — sampling vs instrumentation, flamegraphs, perf events, eBPF profilers, and continuous profiling in prod?"; AGENTS=(Aria Alice Lucidia Shellfish Octavia); OUT="$SAVE_DIR/profiler-eng-$(date +%s).txt"
  echo "🔥 Profiler Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Profiler engineering roundtable complete."; exit 0; fi

if [[ "$1" == "ast-manipulation" ]]; then
  Q="AST manipulation — codemods with jscodeshift/libCST, static analysis, code generation from ASTs, and refactoring at scale?"; AGENTS=(Shellfish Octavia Aria Lucidia Alice); OUT="$SAVE_DIR/ast-manipulation-$(date +%s).txt"
  echo "🌳 AST Manipulation Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ AST manipulation roundtable complete."; exit 0; fi

if [[ "$1" == "treesitter" ]]; then
  Q="Tree-sitter for code intelligence — incremental parsing, writing grammars, syntax highlighting, and query patterns?"; AGENTS=(Alice Lucidia Shellfish Aria Octavia); OUT="$SAVE_DIR/treesitter-$(date +%s).txt"
  echo "🌲 Tree-sitter Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Tree-sitter roundtable complete."; exit 0; fi

if [[ "$1" == "linting-frameworks" ]]; then
  Q="Building custom linting frameworks — AST rules, auto-fixers, plugin architectures, and enforcing team conventions at scale?"; AGENTS=(Octavia Aria Alice Lucidia Shellfish); OUT="$SAVE_DIR/linting-frameworks-$(date +%s).txt"
  echo "🔍 Linting Frameworks Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Linting frameworks roundtable complete."; exit 0; fi

if [[ "$1" == "ide-plugins" ]]; then
  Q="IDE plugin development — VS Code extension APIs, JetBrains plugins, webviews, and shipping great developer tooling?"; AGENTS=(Lucidia Shellfish Alice Octavia Aria); OUT="$SAVE_DIR/ide-plugins-$(date +%s).txt"
  echo "🔌 IDE Plugins Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ IDE plugins roundtable complete."; exit 0; fi

if [[ "$1" == "code-intelligence" ]]; then
  Q="Code intelligence systems — call graphs, type inference, semantic search over code, and building sourcegraph-style tools?"; AGENTS=(Aria Alice Octavia Shellfish Lucidia); OUT="$SAVE_DIR/code-intelligence-$(date +%s).txt"
  echo "🧠 Code Intelligence Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Code intelligence roundtable complete."; exit 0; fi

if [[ "$1" == "compiler-design" ]]; then
  Q="Compiler design for practitioners — lexing, parsing, IR design, SSA form, register allocation, and building a small compiler?"; AGENTS=(Shellfish Lucidia Octavia Alice Aria); OUT="$SAVE_DIR/compiler-design-$(date +%s).txt"
  echo "⚙️ Compiler Design Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Compiler design roundtable complete."; exit 0; fi

if [[ "$1" == "build-systems" ]]; then
  Q="Build system engineering — Bazel, Buck2, Pants, incremental builds, remote caching, and monorepo build performance?"; AGENTS=(Alice Octavia Lucidia Shellfish Aria); OUT="$SAVE_DIR/build-systems-$(date +%s).txt"
  echo "🏗️ Build Systems Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Build systems roundtable complete."; exit 0; fi

if [[ "$1" == "distributed-tracing" ]]; then
  Q="Distributed tracing deep dive — OpenTelemetry, span design, baggage propagation, tail-based sampling, and trace storage?"; AGENTS=(Octavia Aria Lucidia Alice Shellfish); OUT="$SAVE_DIR/distributed-tracing-$(date +%s).txt"
  echo "🔭 Distributed Tracing Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Distributed tracing roundtable complete."; exit 0; fi

if [[ "$1" == "metrics-cardinality" ]]; then
  Q="Metrics cardinality crisis — high-cardinality labels, streaming aggregation, adaptive metrics, and taming Prometheus at scale?"; AGENTS=(Lucidia Alice Shellfish Octavia Aria); OUT="$SAVE_DIR/metrics-cardinality-$(date +%s).txt"
  echo "📊 Metrics Cardinality Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Metrics cardinality roundtable complete."; exit 0; fi

if [[ "$1" == "slo-engineering" ]]; then
  Q="SLO engineering in practice — error budget policies, burn rate alerts, multi-window multi-burn-rate, and SLO as code?"; AGENTS=(Aria Shellfish Octavia Lucidia Alice); OUT="$SAVE_DIR/slo-engineering-$(date +%s).txt"
  echo "🎯 SLO Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ SLO engineering roundtable complete."; exit 0; fi

if [[ "$1" == "ebpf-observability" ]]; then
  Q="eBPF for observability — bpftrace programs, network tracing, CPU/memory profiling, and security monitoring with eBPF?"; AGENTS=(Shellfish Alice Lucidia Octavia Aria); OUT="$SAVE_DIR/ebpf-observability-$(date +%s).txt"
  echo "🔬 eBPF Observability Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ eBPF observability roundtable complete."; exit 0; fi

if [[ "$1" == "continuous-profiling" ]]; then
  Q="Continuous profiling in production — Pyroscope, Parca, always-on sampling, flame graph diffing, and regression detection?"; AGENTS=(Alice Octavia Aria Shellfish Lucidia); OUT="$SAVE_DIR/continuous-profiling-$(date +%s).txt"
  echo "📈 Continuous Profiling Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Continuous profiling roundtable complete."; exit 0; fi

if [[ "$1" == "synthetic-monitoring" ]]; then
  Q="Synthetic monitoring — scripting browser checks, API health probes, multi-step transactions, and alerting on UX metrics?"; AGENTS=(Octavia Lucidia Shellfish Aria Alice); OUT="$SAVE_DIR/synthetic-monitoring-$(date +%s).txt"
  echo "🤖 Synthetic Monitoring Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Synthetic monitoring roundtable complete."; exit 0; fi

if [[ "$1" == "log-aggregation" ]]; then
  Q="Log aggregation at scale — Loki vs Elastic vs ClickHouse for logs, structured logging, log sampling, and cost management?"; AGENTS=(Lucidia Aria Alice Octavia Shellfish); OUT="$SAVE_DIR/log-aggregation-$(date +%s).txt"
  echo "📋 Log Aggregation Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Log aggregation roundtable complete."; exit 0; fi

if [[ "$1" == "alerting-eng" ]]; then
  Q="Alerting engineering — alert fatigue, multi-signal correlation, routing trees, inhibition rules, and on-call ergonomics?"; AGENTS=(Aria Shellfish Alice Lucidia Octavia); OUT="$SAVE_DIR/alerting-eng-$(date +%s).txt"
  echo "🚨 Alerting Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Alerting engineering roundtable complete."; exit 0; fi

if [[ "$1" == "anomaly-detection-ops" ]]; then
  Q="Anomaly detection for ops — statistical baselines, ML-based detection, seasonality, and avoiding alert noise in dynamic systems?"; AGENTS=(Shellfish Octavia Lucidia Alice Aria); OUT="$SAVE_DIR/anomaly-detection-ops-$(date +%s).txt"
  echo "🔍 Anomaly Detection Ops Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Anomaly detection ops roundtable complete."; exit 0; fi

if [[ "$1" == "spark-optimizations" ]]; then
  Q="Spark performance engineering — AQE, broadcast joins, partition skew, catalyst optimizer, and tuning for petabyte datasets?"; AGENTS=(Alice Aria Octavia Shellfish Lucidia); OUT="$SAVE_DIR/spark-optimizations-$(date +%s).txt"
  echo "⚡ Spark Optimizations Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Spark optimizations roundtable complete."; exit 0; fi

if [[ "$1" == "flink-streaming" ]]; then
  Q="Apache Flink deep dive — stateful streaming, exactly-once semantics, checkpointing, watermarks, and CEP patterns?"; AGENTS=(Octavia Lucidia Shellfish Aria Alice); OUT="$SAVE_DIR/flink-streaming-$(date +%s).txt"
  echo "🌊 Flink Streaming Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Flink streaming roundtable complete."; exit 0; fi

if [[ "$1" == "dbt-advanced" ]]; then
  Q="Advanced dbt patterns — macros, packages, unit tests, incremental strategies, semantic layer, and scaling dbt in production?"; AGENTS=(Lucidia Alice Aria Octavia Shellfish); OUT="$SAVE_DIR/dbt-advanced-$(date +%s).txt"
  echo "🔄 dbt Advanced Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ dbt advanced roundtable complete."; exit 0; fi

if [[ "$1" == "data-quality" ]]; then
  Q="Data quality engineering — Great Expectations vs Soda, column-level lineage, quarantine patterns, and quality SLAs?"; AGENTS=(Aria Shellfish Lucidia Alice Octavia); OUT="$SAVE_DIR/data-quality-$(date +%s).txt"
  echo "✅ Data Quality Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Data quality roundtable complete."; exit 0; fi

if [[ "$1" == "airflow-patterns" ]]; then
  Q="Apache Airflow patterns — dynamic DAGs, TaskFlow API, deferrable operators, data-aware scheduling, and Airflow 3 changes?"; AGENTS=(Shellfish Octavia Alice Lucidia Aria); OUT="$SAVE_DIR/airflow-patterns-$(date +%s).txt"
  echo "🌬️ Airflow Patterns Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Airflow patterns roundtable complete."; exit 0; fi

if [[ "$1" == "dagster-assets" ]]; then
  Q="Dagster software-defined assets — asset graph modeling, freshness policies, partitions, and building observable data pipelines?"; AGENTS=(Alice Lucidia Octavia Shellfish Aria); OUT="$SAVE_DIR/dagster-assets-$(date +%s).txt"
  echo "📦 Dagster Assets Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Dagster assets roundtable complete."; exit 0; fi

if [[ "$1" == "data-mesh" ]]; then
  Q="Data mesh architecture — domain ownership, data products, self-serve infrastructure, and federated computational governance?"; AGENTS=(Octavia Aria Shellfish Alice Lucidia); OUT="$SAVE_DIR/data-mesh-$(date +%s).txt"
  echo "🕸️ Data Mesh Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Data mesh roundtable complete."; exit 0; fi

if [[ "$1" == "column-lineage" ]]; then
  Q="Column-level data lineage — OpenLineage, automatic extraction, impact analysis, and governing data transformation chains?"; AGENTS=(Lucidia Alice Shellfish Octavia Aria); OUT="$SAVE_DIR/column-lineage-$(date +%s).txt"
  echo "🔗 Column Lineage Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Column lineage roundtable complete."; exit 0; fi

if [[ "$1" == "mlflow-advanced" ]]; then
  Q="Advanced MLflow patterns — nested runs, custom flavors, model serving, registry webhooks, and MLflow 3 recipes?"; AGENTS=(Aria Shellfish Octavia Lucidia Alice); OUT="$SAVE_DIR/mlflow-advanced-$(date +%s).txt"
  echo "🧪 MLflow Advanced Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ MLflow advanced roundtable complete."; exit 0; fi

if [[ "$1" == "feature-store" ]]; then
  Q="Feature store engineering — online vs offline serving, point-in-time correctness, backfilling, Feast vs Tecton vs Hopsworks?"; AGENTS=(Shellfish Alice Lucidia Aria Octavia); OUT="$SAVE_DIR/feature-store-$(date +%s).txt"
  echo "🗃️ Feature Store Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Feature store roundtable complete."; exit 0; fi

if [[ "$1" == "shadow-deployment" ]]; then
  Q="Shadow mode and champion-challenger deployments — traffic mirroring, offline evaluation, gradual rollout, and safe model releases?"; AGENTS=(Alice Octavia Aria Shellfish Lucidia); OUT="$SAVE_DIR/shadow-deployment-$(date +%s).txt"
  echo "👥 Shadow Deployment Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Shadow deployment roundtable complete."; exit 0; fi

if [[ "$1" == "drift-detection" ]]; then
  Q="ML model drift detection — data drift, concept drift, PSI/KS tests, monitoring frameworks, and automated retraining triggers?"; AGENTS=(Octavia Lucidia Alice Shellfish Aria); OUT="$SAVE_DIR/drift-detection-$(date +%s).txt"
  echo "📉 Drift Detection Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Drift detection roundtable complete."; exit 0; fi

if [[ "$1" == "model-explainability" ]]; then
  Q="Model explainability — SHAP, LIME, attention visualization, counterfactuals, and communicating AI decisions to stakeholders?"; AGENTS=(Lucidia Aria Shellfish Octavia Alice); OUT="$SAVE_DIR/model-explainability-$(date +%s).txt"
  echo "💡 Model Explainability Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Model explainability roundtable complete."; exit 0; fi

if [[ "$1" == "responsible-ai" ]]; then
  Q="Responsible AI engineering — fairness metrics, bias audits, model cards, AI impact assessments, and governance frameworks?"; AGENTS=(Aria Alice Octavia Lucidia Shellfish); OUT="$SAVE_DIR/responsible-ai-$(date +%s).txt"
  echo "⚖️ Responsible AI Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Responsible AI roundtable complete."; exit 0; fi

if [[ "$1" == "idp-patterns" ]]; then
  Q="Internal Developer Platform patterns — golden paths, paved roads, self-service infra, and measuring platform adoption?"; AGENTS=(Shellfish Octavia Aria Lucidia Alice); OUT="$SAVE_DIR/idp-patterns-$(date +%s).txt"
  echo "🛣️ IDP Patterns Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ IDP patterns roundtable complete."; exit 0; fi

if [[ "$1" == "backstage-patterns" ]]; then
  Q="Backstage IDP deep dive — custom plugins, techdocs, scaffolder templates, entity model, and scaling the developer portal?"; AGENTS=(Alice Lucidia Shellfish Octavia Aria); OUT="$SAVE_DIR/backstage-patterns-$(date +%s).txt"
  echo "🎭 Backstage Patterns Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Backstage patterns roundtable complete."; exit 0; fi

if [[ "$1" == "service-catalog" ]]; then
  Q="Service catalog engineering — ownership models, dependency graphs, SLA tracking, incident routing, and keeping it accurate?"; AGENTS=(Octavia Aria Alice Shellfish Lucidia); OUT="$SAVE_DIR/service-catalog-$(date +%s).txt"
  echo "📋 Service Catalog Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Service catalog roundtable complete."; exit 0; fi

if [[ "$1" == "capability-model" ]]; then
  Q="Engineering capability models — DORA metrics, SPACE framework, team topologies maturity, and using data to improve eng org?"; AGENTS=(Lucidia Shellfish Octavia Alice Aria); OUT="$SAVE_DIR/capability-model-$(date +%s).txt"
  echo "📐 Capability Model Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Capability model roundtable complete."; exit 0; fi

if [[ "$1" == "siem-engineering" ]]; then
  Q="SIEM engineering — detection rule authoring, SIGMA rules, alert correlation, log normalization, and SIEM at scale?"; AGENTS=(Shellfish Octavia Lucidia Alice Aria); OUT="$SAVE_DIR/siem-engineering-$(date +%s).txt"
  echo "🔐 SIEM Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ SIEM engineering roundtable complete."; exit 0; fi

if [[ "$1" == "soar-automation" ]]; then
  Q="SOAR playbook automation — incident enrichment, automated containment, alert triage bots, and integrating with SIEM/ticketing?"; AGENTS=(Alice Lucidia Aria Shellfish Octavia); OUT="$SAVE_DIR/soar-automation-$(date +%s).txt"
  echo "🤖 SOAR Automation Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ SOAR automation roundtable complete."; exit 0; fi

if [[ "$1" == "red-team-ops" ]]; then
  Q="Red team operations — adversary simulation, C2 frameworks, living-off-the-land techniques, and building detection engineering?"; AGENTS=(Octavia Shellfish Alice Aria Lucidia); OUT="$SAVE_DIR/red-team-ops-$(date +%s).txt"
  echo "🔴 Red Team Operations Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Red team operations roundtable complete."; exit 0; fi

if [[ "$1" == "incident-response" ]]; then
  Q="Incident response engineering — runbook automation, post-mortem culture, mean-time-to-detect, and building blameless IR processes?"; AGENTS=(Lucidia Alice Octavia Shellfish Aria); OUT="$SAVE_DIR/incident-response-$(date +%s).txt"
  echo "🚨 Incident Response Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Incident response roundtable complete."; exit 0; fi

if [[ "$1" == "vulnerability-mgmt" ]]; then
  Q="Vulnerability management programs — CVSS vs EPSS, risk-based prioritization, SLA tracking, and integrating with CI/CD?"; AGENTS=(Aria Octavia Shellfish Alice Lucidia); OUT="$SAVE_DIR/vulnerability-mgmt-$(date +%s).txt"
  echo "🛡️ Vulnerability Management Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Vulnerability management roundtable complete."; exit 0; fi

if [[ "$1" == "btree-internals" ]]; then
  Q="B-Tree internals — page splits, write amplification, fill factor tuning, and why PostgreSQL uses B-Trees for most indexes?"; AGENTS=(Shellfish Lucidia Octavia Aria Alice); OUT="$SAVE_DIR/btree-internals-$(date +%s).txt"
  echo "🌳 B-Tree Internals Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ B-Tree internals roundtable complete."; exit 0; fi

if [[ "$1" == "lsm-trees" ]]; then
  Q="LSM-Tree storage engines — RocksDB compaction strategies, bloom filters, write vs read amplification, and tuning for workloads?"; AGENTS=(Alice Aria Lucidia Octavia Shellfish); OUT="$SAVE_DIR/lsm-trees-$(date +%s).txt"
  echo "📚 LSM Trees Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ LSM trees roundtable complete."; exit 0; fi

if [[ "$1" == "mvcc-internals" ]]; then
  Q="MVCC deep dive — snapshot isolation, version chains, vacuum/GC, and how Postgres, MySQL, and CockroachDB differ in MVCC?"; AGENTS=(Octavia Shellfish Alice Lucidia Aria); OUT="$SAVE_DIR/mvcc-internals-$(date +%s).txt"
  echo "🔄 MVCC Internals Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ MVCC internals roundtable complete."; exit 0; fi

if [[ "$1" == "query-optimizer" ]]; then
  Q="Query optimizer internals — statistics, cost models, join ordering, adaptive plans, and how to read EXPLAIN ANALYZE output?"; AGENTS=(Lucidia Alice Shellfish Octavia Aria); OUT="$SAVE_DIR/query-optimizer-$(date +%s).txt"
  echo "🔍 Query Optimizer Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Query optimizer roundtable complete."; exit 0; fi

if [[ "$1" == "pg-extensions" ]]; then
  Q="PostgreSQL extensions engineering — writing extensions in C and Rust, hooks API, custom types, operators, and pgxn ecosystem?"; AGENTS=(Aria Octavia Lucidia Shellfish Alice); OUT="$SAVE_DIR/pg-extensions-$(date +%s).txt"
  echo "🐘 PostgreSQL Extensions Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ PostgreSQL extensions roundtable complete."; exit 0; fi

if [[ "$1" == "bgp-engineering" ]]; then
  Q="BGP engineering for internet companies — anycast, traffic engineering, BGP communities, RPKI, and running your own ASN?"; AGENTS=(Shellfish Alice Octavia Lucidia Aria); OUT="$SAVE_DIR/bgp-engineering-$(date +%s).txt"
  echo "🌐 BGP Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ BGP engineering roundtable complete."; exit 0; fi

if [[ "$1" == "dpdk-networking" ]]; then
  Q="DPDK and kernel-bypass networking — userspace packet processing, PMD drivers, DPDK vs io_uring, and building fast packet pipelines?"; AGENTS=(Alice Lucidia Aria Shellfish Octavia); OUT="$SAVE_DIR/dpdk-networking-$(date +%s).txt"
  echo "⚡ DPDK Networking Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ DPDK networking roundtable complete."; exit 0; fi

if [[ "$1" == "ebpf-networking" ]]; then
  Q="eBPF networking — XDP programs, TC hooks, socket filtering, load balancing without kprobes, and replacing iptables with eBPF?"; AGENTS=(Octavia Shellfish Lucidia Alice Aria); OUT="$SAVE_DIR/ebpf-networking-$(date +%s).txt"
  echo "🔬 eBPF Networking Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ eBPF networking roundtable complete."; exit 0; fi

if [[ "$1" == "network-automation" ]]; then
  Q="Network automation — Ansible for networking, NAPALM, Netconf/YANG, network-as-code, and GitOps for network changes?"; AGENTS=(Lucidia Aria Shellfish Octavia Alice); OUT="$SAVE_DIR/network-automation-$(date +%s).txt"
  echo "🤖 Network Automation Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Network automation roundtable complete."; exit 0; fi

if [[ "$1" == "ipv6-migration" ]]; then
  Q="IPv6 migration strategies — dual-stack, NAT64, 464XLAT, addressing schemes, and the engineering challenges of going v6-only?"; AGENTS=(Aria Alice Octavia Shellfish Lucidia); OUT="$SAVE_DIR/ipv6-migration-$(date +%s).txt"
  echo "🌐 IPv6 Migration Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ IPv6 migration roundtable complete."; exit 0; fi

if [[ "$1" == "hexagonal-arch" ]]; then
  Q="Hexagonal architecture in practice — ports, adapters, dependency inversion, testing strategies, and migrating legacy systems?"; AGENTS=(Shellfish Octavia Alice Lucidia Aria); OUT="$SAVE_DIR/hexagonal-arch-$(date +%s).txt"
  echo "⬡ Hexagonal Architecture Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Hexagonal architecture roundtable complete."; exit 0; fi

if [[ "$1" == "reactive-systems" ]]; then
  Q="Reactive systems — the Reactive Manifesto, backpressure, Akka Streams, Project Reactor, and building resilient async systems?"; AGENTS=(Alice Lucidia Shellfish Aria Octavia); OUT="$SAVE_DIR/reactive-systems-$(date +%s).txt"
  echo "⚡ Reactive Systems Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Reactive systems roundtable complete."; exit 0; fi

if [[ "$1" == "choreography-vs-orchestration" ]]; then
  Q="Choreography vs orchestration in microservices — saga patterns, when to centralize vs decentralize workflow, and tracing either approach?"; AGENTS=(Octavia Aria Alice Shellfish Lucidia); OUT="$SAVE_DIR/choreography-vs-orchestration-$(date +%s).txt"
  echo "💃 Choreography vs Orchestration Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Choreography vs orchestration roundtable complete."; exit 0; fi

if [[ "$1" == "distributed-consensus" ]]; then
  Q="Distributed consensus — Raft vs Paxos, leader election, log replication, split-brain prevention, and real-world implementations?"; AGENTS=(Lucidia Shellfish Octavia Alice Aria); OUT="$SAVE_DIR/distributed-consensus-$(date +%s).txt"
  echo "🤝 Distributed Consensus Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Distributed consensus roundtable complete."; exit 0; fi

if [[ "$1" == "crdt-patterns" ]]; then
  Q="CRDTs for distributed systems — G-Counters, OR-Sets, RGA for text, Automerge, and conflict-free collaboration at scale?"; AGENTS=(Aria Alice Lucidia Shellfish Octavia); OUT="$SAVE_DIR/crdt-patterns-$(date +%s).txt"
  echo "🔀 CRDT Patterns Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ CRDT patterns roundtable complete."; exit 0; fi

if [[ "$1" == "spatial-computing" ]]; then
  Q="Spatial computing engineering — Apple Vision Pro, room mapping, hand tracking, eye tracking APIs, and designing immersive UIs?"; AGENTS=(Shellfish Lucidia Aria Octavia Alice); OUT="$SAVE_DIR/spatial-computing-$(date +%s).txt"
  echo "👓 Spatial Computing Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Spatial computing roundtable complete."; exit 0; fi

if [[ "$1" == "xr-engineering" ]]; then
  Q="XR engineering — WebXR, Unity vs Unreal for VR/AR, latency requirements, rendering pipelines, and foveated rendering?"; AGENTS=(Alice Octavia Shellfish Aria Lucidia); OUT="$SAVE_DIR/xr-engineering-$(date +%s).txt"
  echo "🥽 XR Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ XR engineering roundtable complete."; exit 0; fi

if [[ "$1" == "metaverse-infra" ]]; then
  Q="Metaverse infrastructure — persistent world state, spatial partitioning, avatar physics, voice proximity, and thousands concurrent users?"; AGENTS=(Octavia Lucidia Alice Shellfish Aria); OUT="$SAVE_DIR/metaverse-infra-$(date +%s).txt"
  echo "🌐 Metaverse Infrastructure Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Metaverse infrastructure roundtable complete."; exit 0; fi

if [[ "$1" == "ambient-computing" ]]; then
  Q="Ambient computing — always-on sensing, context awareness, proactive AI assistance, and the privacy engineering challenges?"; AGENTS=(Lucidia Shellfish Aria Alice Octavia); OUT="$SAVE_DIR/ambient-computing-$(date +%s).txt"
  echo "🌫️ Ambient Computing Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Ambient computing roundtable complete."; exit 0; fi

if [[ "$1" == "voice-engineering" ]]; then
  Q="Voice interface engineering — ASR pipelines, TTS quality, wake word detection, dialog state machines, and voice UX patterns?"; AGENTS=(Aria Alice Octavia Lucidia Shellfish); OUT="$SAVE_DIR/voice-engineering-$(date +%s).txt"
  echo "🎙️ Voice Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Voice engineering roundtable complete."; exit 0; fi

if [[ "$1" == "computer-vision-eng" ]]; then
  Q="Computer vision engineering — object detection, tracking, YOLO variants, video understanding, and deploying CV models at edge?"; AGENTS=(Shellfish Octavia Lucidia Aria Alice); OUT="$SAVE_DIR/computer-vision-eng-$(date +%s).txt"
  echo "👁️ Computer Vision Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Computer vision engineering roundtable complete."; exit 0; fi

if [[ "$1" == "recommendation-systems" ]]; then
  Q="Recommendation systems — two-tower models, collaborative filtering, real-time features, exploration vs exploitation, and cold start?"; AGENTS=(Alice Lucidia Shellfish Octavia Aria); OUT="$SAVE_DIR/recommendation-systems-$(date +%s).txt"
  echo "💡 Recommendation Systems Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Recommendation systems roundtable complete."; exit 0; fi

if [[ "$1" == "nlp-pipelines" ]]; then
  Q="NLP engineering pipelines — tokenization, NER, relation extraction, text classification, and building scalable NLP services?"; AGENTS=(Octavia Aria Alice Shellfish Lucidia); OUT="$SAVE_DIR/nlp-pipelines-$(date +%s).txt"
  echo "📝 NLP Pipelines Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ NLP pipelines roundtable complete."; exit 0; fi

if [[ "$1" == "multimodal-eng" ]]; then
  Q="Multimodal AI engineering — vision-language models, audio-visual alignment, fusion architectures, and building multimodal APIs?"; AGENTS=(Lucidia Shellfish Octavia Aria Alice); OUT="$SAVE_DIR/multimodal-eng-$(date +%s).txt"
  echo "🎭 Multimodal Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Multimodal engineering roundtable complete."; exit 0; fi

if [[ "$1" == "llm-pretraining" ]]; then
  Q="LLM pretraining engineering — data curation at scale, tokenizer design, training stability, gradient checkpointing, and MFU?"; AGENTS=(Aria Alice Lucidia Shellfish Octavia); OUT="$SAVE_DIR/llm-pretraining-$(date +%s).txt"
  echo "🧠 LLM Pretraining Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ LLM pretraining roundtable complete."; exit 0; fi

if [[ "$1" == "rlhf-engineering" ]]; then
  Q="RLHF and alignment engineering — reward model training, PPO for LLMs, DPO vs RLHF, constitutional AI, and human feedback collection?"; AGENTS=(Shellfish Octavia Aria Alice Lucidia); OUT="$SAVE_DIR/rlhf-engineering-$(date +%s).txt"
  echo "🎯 RLHF Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ RLHF engineering roundtable complete."; exit 0; fi

if [[ "$1" == "gpu-programming" ]]; then
  Q="GPU programming for ML — CUDA kernels, Triton, flash attention, memory hierarchy, and writing custom GPU ops for PyTorch?"; AGENTS=(Alice Lucidia Octavia Shellfish Aria); OUT="$SAVE_DIR/gpu-programming-$(date +%s).txt"
  echo "🖥️ GPU Programming Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ GPU programming roundtable complete."; exit 0; fi

if [[ "$1" == "distributed-training" ]]; then
  Q="Distributed ML training — data vs model parallelism, pipeline parallelism, FSDP, ZeRO optimization, and scaling to thousands of GPUs?"; AGENTS=(Octavia Aria Shellfish Lucidia Alice); OUT="$SAVE_DIR/distributed-training-$(date +%s).txt"
  echo "🚀 Distributed Training Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Distributed training roundtable complete."; exit 0; fi

if [[ "$1" == "kernel-engineering" ]]; then
  Q="Linux kernel engineering — syscall internals, scheduler, memory management, kernel modules, and contributing to the kernel?"; AGENTS=(Lucidia Shellfish Alice Octavia Aria); OUT="$SAVE_DIR/kernel-engineering-$(date +%s).txt"
  echo "⚙️ Kernel Engineering Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Kernel engineering roundtable complete."; exit 0; fi

if [[ "$1" == "lock-free-programming" ]]; then
  Q="Lock-free and wait-free data structures — CAS operations, ABA problem, hazard pointers, memory ordering, and practical applications?"; AGENTS=(Aria Alice Octavia Shellfish Lucidia); OUT="$SAVE_DIR/lock-free-programming-$(date +%s).txt"
  echo "🔓 Lock-Free Programming Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Lock-free programming roundtable complete."; exit 0; fi

if [[ "$1" == "memory-allocators" ]]; then
  Q="Memory allocator design — jemalloc, mimalloc, tcmalloc internals, arena allocation, and building custom allocators for specific workloads?"; AGENTS=(Shellfish Lucidia Aria Alice Octavia); OUT="$SAVE_DIR/memory-allocators-$(date +%s).txt"
  echo "💾 Memory Allocators Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Memory allocators roundtable complete."; exit 0; fi

if [[ "$1" == "simd-vectorization" ]]; then
  Q="SIMD vectorization — AVX-512, auto-vectorization hints, NEON for ARM, and hand-tuning hot loops for 10x throughput?"; AGENTS=(Alice Octavia Lucidia Shellfish Aria); OUT="$SAVE_DIR/simd-vectorization-$(date +%s).txt"
  echo "⚡ SIMD Vectorization Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ SIMD vectorization roundtable complete."; exit 0; fi

if [[ "$1" == "cache-hardware" ]]; then
  Q="CPU cache engineering — cache lines, false sharing, cache-oblivious algorithms, prefetching, and writing cache-friendly code?"; AGENTS=(Octavia Shellfish Alice Lucidia Aria); OUT="$SAVE_DIR/cache-hardware-$(date +%s).txt"
  echo "🧮 Cache Hardware Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Cache hardware roundtable complete."; exit 0; fi

if [[ "$1" == "storage-engines" ]]; then
  Q="Storage engine design — page cache, buffer pool, WAL design, crash recovery, and building a simple storage engine from scratch?"; AGENTS=(Lucidia Alice Shellfish Octavia Aria); OUT="$SAVE_DIR/storage-engines-$(date +%s).txt"
  echo "💽 Storage Engines Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Storage engines roundtable complete."; exit 0; fi

if [[ "$1" == "protocol-design" ]]; then
  Q="Network protocol design — framing, multiplexing, flow control, protocol buffers vs ASN.1, and lessons from designing HTTP/3?"; AGENTS=(Aria Octavia Lucidia Shellfish Alice); OUT="$SAVE_DIR/protocol-design-$(date +%s).txt"
  echo "📡 Protocol Design Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Protocol design roundtable complete."; exit 0; fi

if [[ "$1" == "serialization-formats" ]]; then
  Q="Serialization format engineering — Protobuf, Avro, Arrow, FlatBuffers, zero-copy deserialization, and format evolution strategies?"; AGENTS=(Shellfish Alice Octavia Lucidia Aria); OUT="$SAVE_DIR/serialization-formats-$(date +%s).txt"
  echo "📦 Serialization Formats Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Serialization formats roundtable complete."; exit 0; fi

if [[ "$1" == "chaos-engineering-adv" ]]; then
  Q="Advanced chaos engineering — game days, chaos automation, fault injection in CI, hypothesis-driven chaos, and GameDay culture?"; AGENTS=(Alice Lucidia Aria Octavia Shellfish); OUT="$SAVE_DIR/chaos-engineering-adv-$(date +%s).txt"
  echo "🌪️ Chaos Engineering Advanced Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Chaos engineering advanced roundtable complete."; exit 0; fi

if [[ "$1" == "cost-attribution" ]]; then
  Q="Cloud cost attribution — tagging strategies, showback vs chargeback, per-feature cost tracking, and FinOps team structures?"; AGENTS=(Octavia Shellfish Lucidia Alice Aria); OUT="$SAVE_DIR/cost-attribution-$(date +%s).txt"
  echo "💰 Cost Attribution Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Cost attribution roundtable complete."; exit 0; fi

if [[ "$1" == "sdk-design" ]]; then
  Q="SDK design principles — idiomatic APIs, async patterns, retry/timeout defaults, versioning, and keeping SDKs delightful to use?"; AGENTS=(Lucidia Aria Shellfish Octavia Alice); OUT="$SAVE_DIR/sdk-design-$(date +%s).txt"
  echo "📦 SDK Design Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ SDK design roundtable complete."; exit 0; fi

if [[ "$1" == "cli-design" ]]; then
  Q="CLI tool design — argument parsing, shell completion, interactive prompts, config files, and making CLIs feel great to use?"; AGENTS=(Aria Alice Octavia Shellfish Lucidia); OUT="$SAVE_DIR/cli-design-$(date +%s).txt"
  echo "💻 CLI Design Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ CLI design roundtable complete."; exit 0; fi

if [[ "$1" == "terminal-ui" ]]; then
  Q="Terminal UI engineering — TUI frameworks (Bubble Tea, Charm, Ink), rendering ASCII art, input handling, and building beautiful CLIs?"; AGENTS=(Shellfish Octavia Lucidia Aria Alice); OUT="$SAVE_DIR/terminal-ui-$(date +%s).txt"
  echo "🖥️ Terminal UI Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Terminal UI roundtable complete."; exit 0; fi

if [[ "$1" == "developer-experience" ]]; then
  Q="Developer experience engineering — DX metrics, feedback loops, onboarding time, REPL culture, and measuring DX improvements?"; AGENTS=(Alice Lucidia Shellfish Aria Octavia); OUT="$SAVE_DIR/developer-experience-$(date +%s).txt"
  echo "✨ Developer Experience Roundtable"; echo "$Q" | tee "$OUT"; for AGENT in "${AGENTS[@]}"; do
    REPLY=$(echo "$Q — respond as $AGENT" | ollama run llama3.2 2>/dev/null | head -8); echo "[$AGENT] $REPLY"; echo "[$AGENT]" >> "$OUT"; echo "$REPLY" >> "$OUT"; done
  echo "✅ Developer experience roundtable complete."; exit 0; fi
